...
(global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.MonacoJSXHighlighter = {}));
...
class MonacoJSXHighlighter {
  ...
  runJSXCommentContextAndAction(selection, getAstPromise, onParseErrors = error => error, editor, runJsxCommentAction) {
    getAstPromise = getAstPromise || this.getAstPromise;
    ...
    return new Promise(resolve => {
      if (this._isHighlightBoundToModelContentChanges) {
        ...
        }).catch(error => resolve(runJsxCommentAction(this.getJSXContext(selection, null, editor))) || onParseErrors(error));
      }
    }).catch(error => runJsxCommentAction(this.getJSXContext(selection, null, editor)) || onParseErrors(error));
  }
  addJSXCommentCommand(getAstPromise, onParseErrors = error => error, editor) {
    getAstPromise = getAstPromise || this.getAstPromise;
    editor = editor || this.monacoEditor;
    ...
    this._editorCommandId = editor.addCommand(monaco.KeyMod.CtrlCmd | monaco.KeyCode.US_SLASH, () => {
      ...
      this.runJSXCommentContextAndAction(selection, getAstPromise, onParseErrors, editor, runJsxCommentAction).catch(onParseErrors);
    });
...
exports.configureLocToMonacoRange = configureLocToMonacoRange;
exports["default"] = MonacoJSXHighlighter;
exports.makeJSXTraverse = makeJSXTraverse;
